...
func testTaskToManyProjectTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TaskSlice{&a}
	if err = a.L.LoadProjectTasks(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTaskToManyChildTasks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.ChildID, a.ID)
	queries.Assign(&c.ChildID, a.ID)
	if err = b.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(ctx, tx, boil.Infer()); err != nil {
	...
	check, err := a.ChildTasks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if queries.Equal(v.ChildID, b.ChildID) {
			bFound = true
		}
		if queries.Equal(v.ChildID, c.ChildID) {
			cFound = true
		}
	}
	...
	slice := TaskSlice{&a}
	if err = a.L.LoadChildTasks(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ChildTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ChildTasks = nil
	if err = a.L.LoadChildTasks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ChildTasks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTaskToManyAddOpProjectTasks(t *testing.T) {
	var err error
...
func testTaskToManyAddOpChildTasks(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddChildTasks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.ID, first.ChildID) {
			t.Error("foreign key was wrong value", a.ID, first.ChildID)
		}
		if !queries.Equal(a.ID, second.ChildID) {
		...
		if a.R.ChildTasks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ChildTasks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ChildTasks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testTaskToManySetOpChildTasks(t *testing.T) {
	var err error
	...
	err = a.SetChildTasks(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetChildTasks(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.ChildID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.ChildID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.ChildID) {
		t.Error("foreign key was wrong value", a.ID, d.ChildID)
	}
	if !queries.Equal(a.ID, e.ChildID) {
		t.Error("foreign key was wrong value", a.ID, e.ChildID)
	}
	...
	if a.R.ChildTasks[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.ChildTasks[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testTaskToManyRemoveOpChildTasks(t *testing.T) {
	var err error
	...
	err = a.AddChildTasks(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveChildTasks(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.ChildTasks) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.ChildTasks[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.ChildTasks[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testTaskToOneTaskUsingChild(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.ChildID, foreign.ID)
	...
	if err = local.L.LoadChild(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTaskToOneSetOpTaskUsingChild(t *testing.T) {
	var err error
	...
	for i, x := range []*Task{&b, &c} {
		...
		if x.R.ChildTasks[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.ChildID, x.ID) {
...
func testTaskToOneRemoveOpTaskUsingChild(t *testing.T) {
	var err error
	...
	if err = a.SetChild(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveChild(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.Child().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if !queries.IsValuerNil(a.ChildID) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.ChildTasks) != 0 {
...
func testTaskToOneRemoveOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	taskDBTypes = map[string]string{`ID`: `bigint`, `UserID`: `bigint`, `ChildID`: `bigint`, `Title`: `character varying`, `Note`: `text`, `Status`: `enum.task_status('todo','in_progress','done')`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
