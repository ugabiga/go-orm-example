...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `FirstName`: `character varying`, `LastName`: `character varying`, `Birthday`: `date`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
